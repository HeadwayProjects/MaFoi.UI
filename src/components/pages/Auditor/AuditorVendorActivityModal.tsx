...
const AUDIT_STATUSES = [
  {
    value: AUDIT_STATUS.COMPLIANT,
    label: STATUS_MAPPING[AUDIT_STATUS.COMPLIANT],
  },
  {
    value: AUDIT_STATUS.NON_COMPLIANCE,
    label: STATUS_MAPPING[AUDIT_STATUS.NON_COMPLIANCE],
  },
  {
    value: AUDIT_STATUS.NOT_APPLICABLE,
    label: STATUS_MAPPING[AUDIT_STATUS.NOT_APPLICABLE],
  },
];
...
function AuditorVendorActivityModal({ activity = {}, onClose, onSubmit }: any) {
  ...
  function isInvalid() {
    return (
      ...
      (status.value === ACTIVITY_STATUS.REJECT &&
        !(formsStatusRemarks || "").trim()) ||
      ([AUDIT_STATUS.NON_COMPLIANCE].includes((auditStatus || {}).value) &&
        !(auditRemarks || "").trim()) ||
      ([ACTIVITY_STATUS.APPROVE].includes(status.value) &&
        [AUDIT_STATUS.NOT_APPLICABLE].includes((auditStatus || {}).value) &&
        formsStatusRemarks.trim().length < 20)
    );
  }
  function submit() {
    if (
  ...
  function uploadFile() {
    ...
    api
      .post(`/api/FileUpload/UploadSingleFile?toDoId=${activity.id}`, formData)
      .then(() => {
        setFile(null);
...
export default AuditorVendorActivityModal;
