...
function RuleComplianceDetails({ action, data, onClose, onSubmit }: any) {
    const [form, setForm] = useState<any>({});
    const { states, isFetching: loadingStates } = useGetStates({ ...DEFAULT_OPTIONS_PAYLOAD });
    const [rules, setRules] = useState([]);
    const [compliance, setCompliance] = useState<any>({ hideButtons: true });
    const { updateRuleCompliance, updating } = useUpdateRuleCompliance(({ key, value }: ResponseModel) => {
    ...
    const { createRuleCompliance, creating } = useCreateRuleCompliance(({ key, value }: ResponseModel) => {
        if (key === API_RESULT.SUCCESS) {
