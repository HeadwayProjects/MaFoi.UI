...
import UnBlockModal from "./VendorUnBlockModal";
import BulkUnBlockModal from "./VendorBulkUnBlockModal";
...
function getDefaultPayload() {
  ...
  return [
    ...
    {
      columnName: "fromDate",
      value: dayjs(new Date(fromDate)).local().format(),
    },
    { columnName: "toDate", value: dayjs(new Date(toDate)).local().format() },
    {
      columnName: "status",
      value: [ACTIVITY_STATUS.OVERDUE, ACTIVITY_STATUS.REJECTED].join(
        API_DELIMITER
      ),
    },
  ];
...
function VendorLockUnLock() {
  const [breadcrumb] = useState(
    GetVendorAuditScheduleBreadcrumb("Un-Block Activities")
  );
        ...
        <form className="p-0 mx-3 my-2">
          <div className="card shadow border-0 p-2 mt-2 mb-3 filters">
            <div className="d-flex flex-row m-0 align-items-end">
              ...
              <div>
                <AdvanceSearch
                  fields={[FILTERS.MONTH]}
                  payload={getAdvanceSearchPayload()}
                  onSubmit={search}
                />
              </div>
              {hasUserAccess(
                USER_PRIVILEGES.AUDIT_SCHEDULE_UNBLOCK_BLOCKED
              ) && (
...
export default VendorLockUnLock;
