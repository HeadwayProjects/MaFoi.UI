...
import { useImportVendorAuditSchedule } from "../../../../backend/masters";
...
function VendorAuditScheduleImportModal({ onClose }: any) {
  const [apiError, setApiError] = useState<any>(null);

  const { importVendorAuditSchedule, uploading } = useImportVendorAuditSchedule(
    (response: any) => {
  ...
  function handleSubmit({ file }: any) {
    ...
    importVendorAuditSchedule(formData);
  }
    ...
    <>
      ...
      >
        <Modal.Header closeButton={true} onHide={onClose}>
          <Modal.Title className="bg">Import Vendor Audit Schedule</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="d-flex flex-column" style={{ minHeight: "200px" }}>
            {Boolean(apiError) && (
              <Alert variant="danger" className="mx-4 my-2">
                There are few errors identified in the file uploaded. Correct
                the errors and upload again.{" "}
                <a href="/" onClick={downloadErros}>
                  Click here
                </a>{" "}
                to download the errors.
              </Alert>
            )}
...
export default VendorAuditScheduleImportModal;
