...
import { useUpdateVendorAuditSchedule } from "../../../../backend/masters";
...
function VendorUnBlockModal({ activity = {}, onClose, onSubmit }: any) {
  const [newDueDate, setNewDueDate] = useState<any>(new Date(activity.dueDate));
  const { updateAuditSchedule, updating } = useUpdateVendorAuditSchedule(
    ({ status, message }: any) => {
...
export default VendorUnBlockModal;
