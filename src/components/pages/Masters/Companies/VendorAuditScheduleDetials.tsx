...
import { useGetAllVendorActivities } from "../../../../backend/query";
import {
  ACTIVITY_TYPE,
  ACTIVITY_TYPE_ICONS,
  API_DELIMITER,
...
import AdminLocations from "./AdminVendorScheduleLocation";
import ConfirmModal from "../../../common/ConfirmModal";
import { useDeleteVendorAuditSchedule } from "../../../../backend/masters";
...
function VendorAuditScheduleDetails(this: any) {
  const [breadcrumb] = useState(
    GetVendorAuditScheduleBreadcrumb("Audit Schedule Details")
  );
  ...
  const [statusFilters] = useState([
    {
      columnName: "status",
      value: [ACTIVITY_STATUS.PENDING, ACTIVITY_STATUS.OVERDUE].join(
        API_DELIMITER
      ),
    },
  ]);
  ...
  const { deleteAuditSchedule, deleting } = useDeleteVendorAuditSchedule(
    ({ key, value }: any) => {
  ...
  function hasFilters(ref: any, field = "companyId") {
    const _filters =
      (ref ? ref.current : { ...(payloadRef.current || {}) }.filters) || [];
  ...
  function onLocation2Change(event: any) {
    const {
      company,
      associateCompany,
      locationId,
  ...
  function performBulkDelete() {
    deleteAuditSchedule(selectedRows.map((x: any) => x.id));
  }
  ...
  return (
    <>
      <MastersLayout
        title="Vendor Audit Schedule Details"
          ...
          <div className="card shadow border-0 p-2 mt-2 mb-3 filters">
            <div className="d-flex flex-row m-0 align-items-end">
              ...
              <div className="ms-auto">
                {hasUserAccess(
                  USER_PRIVILEGES.DELETE_AUDIT_SCHEDULE_DETAILS
                ) && (
                  <button
      ...
      {action === ACTIONS.BULK_DELETE && (
        <ConfirmModal
          ...
          onSubmit={performBulkDelete}
          onClose={() => setAction(ACTIONS.NONE)}
        >
          <p className="text-center mb-4">
            There are {selectedRows.length} record(s) selected for deleting.
          </p>
          <div className="text-center mb-4">
...
export default VendorAuditScheduleDetails;
