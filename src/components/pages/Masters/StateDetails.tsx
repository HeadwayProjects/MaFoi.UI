...
function StateDetails({ action, data, onClose, onSubmit }: any) {
    ...
    const { createState } = useCreateState((response: any) => {
    ...
    function submitState(e: any) {
        preventDefault(e);
        if (form.valid) {
            const { code, name } = form.values;
            const existingData = states.filter((x: any) => x.id !== (data || {}).id);
