...
import AuditorVendorActivityModal from "../../Auditor/AuditorVendorActivityModal";
import EditActivityModal from "./EditVendorActivityModal";
...
import PublishModal from "../../Auditor/PublishModal";
import Table, {
  CellTmpl,
  DEFAULT_PAYLOAD,
  TitleTmpl,
...
import {
  useExportVendorTodos,
  useGetAllVendorActivities,
} from "../../../../backend/query";
...
import VendorBulkUploadModal from "./VendorToDoBulkModal";
import AdminVendorLocations from "../../Masters/Companies/AdminVendorLocation";

const STATUS_BTNS = [
  {
    name: ACTIVITY_STATUS.ACTIVITY_SAVED,
    label: STATUS_MAPPING[ACTIVITY_STATUS.ACTIVITY_SAVED],
    style: "secondary",
  },
  {
    name: ACTIVITY_STATUS.PENDING,
    label: STATUS_MAPPING[ACTIVITY_STATUS.PENDING],
    style: "warning",
  },
  {
    name: ACTIVITY_STATUS.OVERDUE,
    label: STATUS_MAPPING[ACTIVITY_STATUS.OVERDUE],
    style: "danger",
  },
  {
    name: ACTIVITY_STATUS.SUBMITTED,
    label: STATUS_MAPPING[ACTIVITY_STATUS.SUBMITTED],
    style: "danger",
  },
  {
    name: ACTIVITY_STATUS.REJECTED,
    label: STATUS_MAPPING[ACTIVITY_STATUS.REJECTED],
    style: "danger",
  },
  {
    name: ACTIVITY_STATUS.AUDITED,
    label: STATUS_MAPPING[ACTIVITY_STATUS.AUDITED],
    style: "danger",
  },
];
...
function getAdvanceSearch(state: any) {
  ...
  if ((state || {}).fromDate) {
    filters.push({
      columnName: "fromDate",
      value: dayjs(state.fromDate).startOf("D").toISOString(),
    });
    filters.push({
      columnName: "toDate",
      value: dayjs(state.toDate).endOf("D").toISOString(),
    });
  }
...
function VendorLocationActivitiesManagement() {
  ...
  const { activities, total, isFetching, refetch } = useGetAllVendorActivities(
    payload,
    Boolean(hasFilters(null, "companyId"))
  );
  const [publish, setPublish] = useState(false);
  const [selectedRows, setSelectedRows] = useState<any[]>([]);
  const [alertMessage, setAlertMessage] = useState<any>(null);
  const { auditReportVendor, exporting } = useAuditReportForVendor(
    (response: any) => {
      if (response) {
        downloadFileContent({
          name: getAuditReportFileName(
            data.data[0],
  ...
  const { exportTodos, exporting: exportingTodos } = useExportVendorTodos(
    (response: any) => {
      downloadFileContent({
        name: "Audit Vendor Activities.xlsx",
        type: response.headers["content-type"],
        content: response.data,
      });
    },
  ...
  function publishActivity(e: any) {
    ...
    if (!Boolean(_filter)) {
      ...
      setSubmitting(true);
      const _request = {
        ...reduceArraytoObj(lfRef.current),
        ...reduceArraytoObj(afRef.current),
      };
      const _payload = {
        companyId: _request.companyId,
        associateCompanyId: _request.associateCompanyId,
        locationId: _request.locationId,
        vendorCategoriesId: _request.vendorCategoriesId,
        vendorRegistrationId: _request.vendorRegistrationId,
        month: _request.month,
        year: _request.year,
        statuses: STATUS_BTNS.map((x) => x.name),
      };
      api
        .post("/api/ToDoVendor/GetToDoVendorByCriteria", _payload)
        .then((response) => {
          if (response && response.data) {
            const _rows = response.data || [];
            const auditableRows = _rows.filter((x: any) =>
              [ACTIVITY_TYPE.AUDIT, ACTIVITY_TYPE.PHYSICAL_AUDIT].includes(
                x.auditted
              )
            );
            const submittedRows = auditableRows.filter((x: any) =>
              [
                ACTIVITY_STATUS.SUBMITTED,
                ACTIVITY_STATUS.PENDING,
                ACTIVITY_STATUS.OVERDUE,
                ACTIVITY_STATUS.ACTIVITY_SAVED,
              ].includes(x.status)
            );
  ...
  function onPublish(e: any, recommendations: any) {
    ...
    api
      .post("/api/AuditorVendor/UpdateVendorPublishStatus", _payload)
      .then((response) => {
        refetch();
        setPublish(false);
  ...
  function onLocationChange(event: any) {
    ...
    setLocationFilter([
      ...
      { columnName: "locationId", value: locationId },
      { columnName: "vendorCategoriesId", value: vendorCategoriesId },
      { columnName: "vendorRegistrationId", value: vendorRegistrationId },
    ]);
  }

  function editActivity(activity: any) {
    setActivity(ACTIONS.EDIT);
  ...
  function downloadForm(activity: any) {
    setSubmitting(true);
    api
      .get(`/api/ActStateMapping/Get?id=${activity.actStateMappingId}`)
      .then((response) => {
  ...
  function downloadReport(event: any) {
    ...
    setSubmitting(true);
    const _request = {
      ...reduceArraytoObj(lfRef.current as any),
      ...reduceArraytoObj(afRef.current),
    };
        ...
        if (response && response.data) {
          ...
          if (_report.length > 0) {
            ...
            const _p = {
              ...
              company: _payload.companyId,
              location: _payload.locationId,
              associateCompany: _payload.associateCompanyId,
            };
            delete _p.companyId;
            delete _p.associateCompanyId;
            delete _p.locationId;
            auditReportVendor(_p);
          } else {
            toast.warn(
              "There are no reports available for the selected month and year."
            );
          }
        }
      })
    ...
    if (!Boolean(_filter)) {
      ...
      api
        ...
        .then((response) => {
          if (response && response.data) {
            const _rows = response.data || [];
            const _applicableRows = _rows.filter(
              (x: any) =>
                x.auditted === ACTIVITY_TYPE.AUDIT &&
                x.status !== ACTIVITY_STATUS.AUDITED &&
                x.status !== ACTIVITY_STATUS.SUBMITTED
            );
            const _published = _applicableRows.filter((x: any) => x.published);
            if (_published.length > 0) {
              setAlertMessage(
                "All the activities are published for the selected month and year. See Audit report for more details."
              );
  ...
  function onSubmitToAuditorHandler(e: any) {
    ...
    api
      .post("/api/ToDoVendor/SubmitToVendorAudit", ids)
      .then(() => {
  ...
  function bulkFileDownloadthroughZip(event: any) {
    ...
    const selectedStatuses = Object.keys(checkedStatuses)
      .filter((status) => checkedStatuses[status] === true)
      .join("-");
    ...
    const _payload = {
      ...
      vendorCategory: _request.vendorCategoriesId,
      vendorRegistrationId: _request.vendorRegistrationId,
    };
    ...
    if (_idData.length > 0) {
      const fullPayload = {
        toDoVendorIds: _idData,
        searchParamsforToDoVendors: {
          company: _request.companyId,
          associateCompany: _request.associateCompanyId,
          location: _request.locationId,
          vendorCategory: _request.vendorCategory,
          vendorRegistration: _request.vendorRegistrationId,
          month: _request.month ? _request.month : "",
          year: _request.year ? _request.year : 0,
          statuses: finalStatuses ? finalStatuses : " ",
        },
      };
      api
        .post(
          "/api/ToDoVendorDetails/GetByToDowithZipWithOnlyFileforMultipleIds",
          fullPayload,
          {
      ...
      <div className={`nav-item dropdown ${styles.tableActions}`}>
        ...
        <div className="dropdown-menu">
          ...
          <a
            ...
            href="/"
            onClick={bulkFileDownloadthroughZip}
          >
            All Files Download
          </a>
    ...
    return (
      <div className="d-flex align-items-center position-relative">
        {!!row.formsStatusRemarks && (
          <OverlayTrigger
            overlay={<Tooltip>{row.formsStatusRemarks}</Tooltip>}
            placement="bottom"
        ...
        <span className={`status-${status} ellipse`}>
  ...
  function AuditStatusTmpl({ cell }: any) {
    ...
    return (
      <div className="d-flex align-items-center position-relative">
        {!!row.auditRemarks && (
          <OverlayTrigger
            overlay={<Tooltip>{row.auditRemarks}</Tooltip>}
            placement="bottom"
            delay={{ show: TOOLTIP_DELAY } as any}
          >
            <FontAwesomeIcon icon={faInfoCircle} className="me-2" />
        ...
        {status && (
          <span className={`${color[status]} ellipse`}>
            {STATUS_MAPPING[status] || status}
          </span>
        )}
      </div>
    ...
    return (
      <div className="d-flex flex-row align-items-center position-relative">
        <Icon
          ...
          type="button"
          name={
            hasUserAccess(USER_PRIVILEGES.SUBMITTER_VENDOR_ACTIVITIES_UPLOAD) ||
            hasUserAccess(USER_PRIVILEGES.REVIEWER_VENDOR_ACTIVITIES_AUDIT)
              ? "pencil"
              : "eye"
          }
          text={
  ...
  function hasFilters(ref: any, field = "companyId") {
    const _filters: any =
      (ref
        ? ref.current
        : ({ ...(payloadRef.current || {}) } as any)["filters"]) || [];
    ...
    <>
      <div className="d-flex flex-column">
        <div className="d-flex  p-2 align-items-center pageHeading shadow">
          <h4 className="mb-0">Vendor Audit Compliances</h4>
          <div className="d-flex align-items-end h-100">
            <nav aria-label="breadcrumb">
              <ol className="breadcrumb mb-0 d-flex justify-content-end">
                ...
                <li className="breadcrumb-item  fw-bold active">
                  Audit Compliances
                </li>
              </ol>
            </nav>
        ...
        <form className="p-0 mx-3 my-2">
          <div className="card shadow border-0 p-2 mt-2 mb-3">
            <div className="d-flex flex-row filters">
              <AdminVendorLocations onChange={onLocationChange} />
              <div className="col-5">
                <AdvanceSearch
                  fields={[FILTERS.MONTH, FILTERS.DUE_DATE]}
                  payload={getAdvanceSearchPayload()}
                  onSubmit={search}
                  downloadReport={
                    hasUserAccess(
                      USER_PRIVILEGES.REVIEWER_ACTIVITIES_DOWNLOAD_REPORT
                    ) ||
                    hasUserAccess(
                      USER_PRIVILEGES.SUBMITTER_ACTIVITIES_DOWNLOAD_REPORT
                    )
                      ? downloadReport
                      : undefined
                  }
                ...
                {statusBtns.map((btn) => {
                  return (
                    <div className="mx-2" key={btn.name}>
                      ...
                      />
                      <label
                        className={`btn bg-status-${btn.name}`}
                        htmlFor={btn.name}
                      >
                        {btn.label}
                      </label>
      ...
      {vendorUpload && (
        <VendorBulkUploadModal
          onClose={handleVendorBulkUploadClose}
...
export default VendorLocationActivitiesManagement;
