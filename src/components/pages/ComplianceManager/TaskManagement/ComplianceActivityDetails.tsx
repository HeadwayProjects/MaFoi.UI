...
import { useGetComplianceActivityDocuments, useGetComplianceById, useUpdateComplianceSchedule, useUploadDocument } from "../../../../backend/compliance";
...
export default function ComplianceActivityDetails(this: any, { data, onCancel, onSubmit }: any) {
    ...
    const [editable] = useState(true)//useState(data.status === ComplianceActivityStatus.SUBMITTED && hasUserAccess(USER_PRIVILEGES.MANAGER_ACTIVITIES_REVIEW));
    ...
    const { updateComplianceSchedule } = useUpdateComplianceSchedule((response: any) => {
                    ...
                    <div className="card border-0 h-full w-full">
                        ...
                        <div className={`d-flex flex-column justify-space-between p-4 ${styles.activitydetailsContainer} `}>
                            ...
                            <div className="d-flex justify-content-end mt-4">
                                {
                                    editable && hasUserAccess(USER_PRIVILEGES.MANAGER_ACTIVITIES_REVIEW) &&
                                    <>
                                        <Button variant="outline-secondary" className="btn btn-outline-secondary px-4" onClick={onCancel}>{'Cancel'}</Button>
                                        <Button variant="primary" onClick={handleSubmit} disabled={!form.valid} className="px-4 ms-3" >{'Submit'}</Button>
                                    </>
                                }
                                {
                                    (!editable || !hasUserAccess(USER_PRIVILEGES.MANAGER_ACTIVITIES_REVIEW)) &&
